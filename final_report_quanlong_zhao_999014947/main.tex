...
\newcommand{\projecttitle}{VLA Robotic Fish: Underwater Target Navigation with Visual Fusion and GRU Imitation}
\newcommand{\studentname}{Quanlong Zhao}
\newcommand{\studentid}{999014947}
\newcommand{\successbest}{95.7\%}

\title{\textbf{\projecttitle}\\
\large Final Report for \projectname}
...
\centering
\vspace*{0.75in}
{\Huge \textbf{VLA Robotic Fish}\par}
\vspace{0.2in}
{\Large Underwater Target Navigation with Visual Fusion and GRU Imitation\par}
\vspace{0.15in}
{\large Final Report for \projectname\par}
\vspace{0.6in}
...
outcome of \textit{\projecttitle}, the GitHub-ready \projectname\ project for
an autonomous robotic fish in NVIDIA Isaac Sim. The project goal is a
...
The uploaded repository is:

\begin{center}
\url{https://github.com/sonjuonr/Visual_Fusion_GRU}
\end{center}
...
datasets, report files, and upload notes. The full Isaac Sim installation and
local TensorBoard event folders are not intended to be the core public
artifact.

\subsection{GitHub Package Structure}

The report follows the same structure that was uploaded to GitHub. The main
public package is organized as:
...
Visual_Fusion_GRU/
  README.md
  requirements.txt
  src/                         Environment, VLA, RL, and IL source code
  src/imitation/               BC, DAgger, dataset, policy, model modules
  configs/imitation/           Teacher, BC, DAgger, GRU, evaluation configs
  scripts/                     Training, evaluation, and release helpers
  models/                      Compact model checkpoints and run configs
  monitor_logs/                RL and fallback teacher monitor CSV logs
  imitation_data/              Teacher seed datasets and eval summaries
  imitation_runs/              DAgger datasets, checkpoints, run summaries
  artifacts_manifest/          Included files and excluded checkpoint lists
  docs/                        GitHub upload notes, assets, next experiments
  final_report_quanlong_zhao_999014947/
    main.tex
    Quanlong_Zhao_999014947_Final_Report.pdf
    figures/
    build_figures.py
\end{verbatim}
}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.98\linewidth]{figures/training_pipeline.pdf}
\caption{Research pipeline: the original RL alpha-fusion route exposed the semantic-control bottleneck, then the project moved to a pure-CLIP GRU imitation route.}
\label{fig:pipeline}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.98\linewidth]{figures/imitation_learning_structure.pdf}
\caption{Imitation-learning structure from BC to MLP DAgger to GRU DAgger and hard-case GRU fine-tuning.}
\label{fig:imitation-structure}
\end{figure}
...
\midrule
\endhead
GitHub repository & \url{https://github.com/sonjuonr/Visual_Fusion_GRU} \\
...
\begin{itemize}
  ...
  \item \path{figures/imitation_learning_structure.pdf}: generated BC-to-DAgger-to-GRU structure figure.
