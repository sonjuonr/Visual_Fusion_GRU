...
& (Join-Path $repoRoot "python.bat") `
    ...
    --resume-from (Join-Path $repoRoot "my_projects\checkpoints\fish_vla_phaseb_1069200_steps.zip") `
    --learning-rate 1e-4 `
    --model-output-path (Join-Path $repoRoot "my_projects\models\my_underwater_robot_policy_phaseb_slowalpha_resume_cont") `
    --tb-run-name fish_vla_phaseb_slowalpha_resume_cont `
    --monitor-log-path (Join-Path $repoRoot "my_projects\monitor_logs\monitor_phaseb_slowalpha_resume_cont.monitor.csv") `
    --timesteps 300000 `
    --alpha-start 0.95 `
    --alpha-max 1.0 `
